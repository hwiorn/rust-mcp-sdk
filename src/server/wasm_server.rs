...
use serde_json::json;
use crate::types::{
    CallToolParams, CallToolResult, ClientRequest, Content, GetPromptParams,
    GetPromptResult, Implementation, InitializeParams, InitializeResult,
    JSONRPCError, JSONRPCResponse, ListPromptsParams, ListPromptsResult,
    ListResourcesParams, ListResourcesResult, ListToolsParams, ListToolsResult,
    PromptInfo, ReadResourceParams, ReadResourceResult,
...
impl WasmMcpServer {
    ...
    fn handle_list_tools(&self, _params: ListToolsParams) -> Result<Value> {
        let tools: Vec<ToolInfo> = self.tools
            .values()
            .map(|tool| tool.info())
            .collect();
    ...
    fn handle_list_prompts(&self, _params: ListPromptsParams) -> Result<Value> {
        let prompts: Vec<PromptInfo> = self.prompts
            .values()
...
impl WasmMcpServerBuilder {
    ...
    pub fn resource<R: WasmResource + 'static>(mut self, name: impl Into<String>, resource: R) -> Self {
