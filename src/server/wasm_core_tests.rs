...
mod tests {
    ...
    use crate::types::{Request, RequestId, ClientRequest};
    ...
    async fn test_initialize() {
        ...
        let init_request = Request::Client(Box::new(serde_json::from_value(json!({
            "method": "initialize",
            "params": {
        ...
        let response = server.handle_request(RequestId::from(1i64), init_request).await;
        
        // Check response structure
        assert_eq!(response.jsonrpc, "2.0");
        assert!(matches!(response.payload, crate::types::jsonrpc::ResponsePayload::Result(_)));
    ...
    async fn test_tools_list_uninitialized() {
        ...
        let list_request = Request::Client(Box::new(serde_json::from_value(json!({
            "method": "tools/list",
            "params": {}
        })).unwrap()));
        
        let response = server.handle_request(RequestId::from(1i64), list_request).await;
        
        // Should return error when not initialized
        assert!(matches!(response.payload, crate::types::jsonrpc::ResponsePayload::Error(_)));
    ...
    async fn test_tools_list_after_init() {
        ...
        server.handle_request(RequestId::from(1i64), init_request).await;
        ...
        let response = server.handle_request(RequestId::from(2i64), list_request).await;
    ...
    async fn test_tool_call_success() {
        ...
        // Call tool
        let call_request = Request::Client(Box::new(serde_json::from_value(json!({
            "method": "tools/call",
            "params": {
                "name": "add",
                "arguments": {
        ...
        let response = server.handle_request(RequestId::from(2i64), call_request).await;
